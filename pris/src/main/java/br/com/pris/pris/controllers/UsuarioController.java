...
public class UsuarioController {
    ...
    @GetMapping("/{email}")
	public ResponseEntity<Integer> showId(@PathVariable String email) {
		return ResponseEntity.ok(usuarioService.loadUserByEmail(email));
	}
    
    @PutMapping("/{email}")
	public ResponseEntity<Usuario> updateUser(@Valid @RequestBody Usuario usuario, @PathVariable String email) {
    	String senhaCriptografada = passwordEncoder.encode(usuario.getSenha());
        usuario.setSenha(senhaCriptografada);
		return ResponseEntity.ok(usuarioService.changeUsuario(usuario, email));
	}
}
